...
var Resource = function (mbid) {
	...
	this.readData = function (linkedEntities, data) {
		for(var l = 0; l < Object.keys(linkedEntities).length; l++){
			var linkedEntity = Object.keys(linkedEntities)[l];
			if (typeof this._readDataFunctions[linkedEntities[linkedEntity]] === 'function') {
				this._readDataFunctions[linkedEntities[linkedEntity]](data);
			}
		}
	};
...
mb.search = function(resource, query, filter, force, callback){
	...
	if(filter instanceof Object) {
		for(var k = 0; k < Object.keys(filter).length; k++){
			var key = Object.keys(filter)[k];
