...
var ReleaseGroupLinkedEntities = function () {
	...
	this.loadReleaseGroupList = function (data) {
		...
		for (var i = 0; i < releaseGroups.length; i++) {
			...
			releaseGroup.setProperty('primary-type', releaseGroups[i]['primary-type']);
			releaseGroup.setProperty('secondary-type-list', releaseGroups[i]['secondary-type-list']);
...
mb.lookupRelease = function (mbid, linkedEntities, force, callback) {
	...
	mb.lookup('release', mbid, linkedEntities, function (err, data) {
		if (err) {
			callback(err, data);
			return;
		}
