var request = require('request'),
    ...
    Trickle = require('timetrickle').Trickle,
    os = require('os'),
    timers = require('timers'),
    trickle = new Trickle(1, 1000);
...
var Error = function (data) {
	Entity.call(this);
	this._dataProperties = ['statusCode', 'message'];
	this.message = null;
...
mb.search = function(resource, query, force, callback){
	if (typeof callback === 'undefined') {
		callback = force;
		force = false;
	}
	var uri = mbBaseURI + resource + '?query=' + encodeURIComponent(query);

	var count = 0;
	var search = function (callback) {
		trickle.trickle(1, function (err) {
		count++;
			request({
				'method' : 'GET', 
				'uri' : uri, 
...
mb.searchReleases = function (query, force, callback) {
	if (typeof callback === 'undefined') {
		callback = force;
		force = false;
	}

	mb.search('release', query, force, function (err, data) {
		if (err) {
			callback(err, data);
			return;
		}
...
mb.searchRecordings = function (query, force, callback) {
	if (typeof callback === 'undefined') {
		callback = force;
		force = false;
	}

	mb.search('recording', query, function (err, data) {
		if (err) {
			callback(err, data);
			return;
		}
...
mb.searchArtists = function (query, force, callback) {
	if (typeof callback === 'undefined') {
		callback = force;
		force = false;
	}

	mb.search('artist', query, function (err, data) {
		if (err) {
			callback(err, data);
			return;
		}
